...
func TestStage_AppendsStageAuditEvents(t *testing.T) {
	...
	if _, err := runStageCmd(t, testWorkitemID, "current", false); err != nil {
		t.Fatalf("stage move: %v", err)
	}
	...
	first := events[0]
	if first.Event != wkaudit.EventStage || first.ID != testWorkitemID {
		t.Fatalf("first event unexpected: %+v", first)
	}
	if first.From != "none" || first.To != "next" {
		t.Fatalf("first event from/to = %q/%q, want none/next", first.From, first.To)
	}
	if first.Type != "design" {
		t.Fatalf("first event type = %q, want design", first.Type)
	}

	second := events[1]
	if second.From != "next" || second.To != "current" {
		t.Fatalf("second event from/to = %q/%q, want next/current", second.From, second.To)
	}

	third := events[2]
	if third.From != "current" || third.To != "none" {
		t.Fatalf("third event from/to = %q/%q, want current/none", third.From, third.To)
	}
}

func TestGroup_AppendsGroupAuditEvents(t *testing.T) {
	root := linkTestCampaign(t)
	restore := chdir(t, root)
	defer restore()
	...
	first := events[0]
	if first.Event != wkaudit.EventGroup || first.ID != testWorkitemID {
		t.Fatalf("first event unexpected: %+v", first)
	}
	if first.From != "none" || first.To != "camp-workflow" {
		t.Fatalf("first event from/to = %q/%q, want none/camp-workflow", first.From, first.To)
	}
	if first.Type != "design" {
		t.Fatalf("first event type = %q, want design", first.Type)
	}
	...
	second := events[1]
	if second.From != "camp-workflow" || second.To != "none" {
		t.Fatalf("second event from/to = %q/%q, want camp-workflow/none", second.From, second.To)
	}
}
