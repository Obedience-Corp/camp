...
// loadForMutation resolves the quest and loads its checklist for a write.
func (s *Service) loadForMutation(ctx context.Context, identifier string) (*Quest, *Checklist, string, error) {
	if err := s.ensureInitialized(); err != nil {
		return nil, nil, "", err
	}
	q, err := Resolve(ctx, s.campaignRoot, identifier)
	...
	return q, cl, path, nil
}

func (s *Service) saveResult(ctx context.Context, q *Quest, cl *Checklist, path string, item *ChecklistItem) (*ChecklistMutationResult, error) {
...
func (s *Service) AddChecklistItem(ctx context.Context, identifier string, opts AddChecklistItemOptions) (*ChecklistMutationResult, error) {
	q, cl, path, err := s.loadForMutation(ctx, identifier)
	if err != nil {
		return nil, err
	}
	...
	stored := cl.Add(item)
	return s.saveResult(ctx, q, cl, path, stored)
}
...
func (s *Service) SetChecklistItemStatus(ctx context.Context, identifier, selector string, status ChecklistItemStatus) (*ChecklistMutationResult, error) {
	...
	return s.saveResult(ctx, q, cl, path, item)
}
...
// RankChecklistItem sets an item's explicit sort rank.
