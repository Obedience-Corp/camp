...
var (
	...
	ErrSchemaExists = fmt.Errorf("workflow already initialized: %w", camperrors.ErrAlreadyExists)

	// ErrInvalidSchema is returned when the schema file is malformed.
	ErrInvalidSchema = fmt.Errorf("invalid workflow schema: %w", camperrors.ErrInvalidInput)
	...
	ErrItemNotFound = fmt.Errorf("item not found: %w", camperrors.ErrNotFound)
	...
	ErrStatusNotFound = fmt.Errorf("status not found: %w", camperrors.ErrNotFound)

	// ErrAlreadyExists is returned when an item already exists at the destination.
	ErrAlreadyExists = fmt.Errorf("item already exists at destination: %w", camperrors.ErrAlreadyExists)
