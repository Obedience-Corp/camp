...
func TestHasLegacyDungeons(t *testing.T) {
	tests := []struct {
		name  string
		setup func(t *testing.T, root string)
		want  bool
	}{
		{name: "empty campaign", setup: func(t *testing.T, root string) {}, want: false},
		{
			name:  "fully hidden campaign",
			setup: func(t *testing.T, root string) { mustMkdir(t, filepath.Join(root, Hidden)) },
			want:  false,
		},
		{
			name:  "visible root dungeon",
			setup: func(t *testing.T, root string) { mustMkdir(t, filepath.Join(root, Visible)) },
			want:  true,
		},
		{
			name: "visible dungeon only in a workflow type",
			setup: func(t *testing.T, root string) {
				mustMkdir(t, filepath.Join(root, Hidden))
				mustMkdir(t, filepath.Join(root, "workflow", "design", Visible))
			},
		...
		{
			name: "a project dungeon is not campaign legacy",
			setup: func(t *testing.T, root string) {
				mustMkdir(t, filepath.Join(root, Hidden))
				mustMkdir(t, filepath.Join(root, ProjectsDir, "camp", "internal", "dungeon"))
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := HasLegacyDungeons(context.Background(), root)
			if err != nil {
				t.Fatalf("HasLegacyDungeons() error = %v", err)
			}
			if got != tt.want {
				t.Errorf("HasLegacyDungeons() = %v, want %v", got, tt.want)
			}
		})
	}
}
