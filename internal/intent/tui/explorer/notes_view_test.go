...
func TestGroupNotes_SplitsActiveAndArchived(t *testing.T) {
	...
	groups := groupNotes(notes, nil, nil, false)
	if len(groups) < 2 {
...
func TestGroupExplorerItemsByStatus_PutsNotesAfterActive(t *testing.T) {
	...
	groups2 := groupExplorerItemsByStatus(nested, nil, false, map[string]bool{
...
func TestConvert_TUIFlow_NoteBecomesIntent(t *testing.T) {
	...
	m.ready = true
	m.groups = groupExplorerItemsByStatus([]*intent.Intent{note}, nil, false, map[string]bool{"notes": true}, false)
...
func TestArchive_TUIFlow_NoteMovesToArchived(t *testing.T) {
	...
	m.ready = true
	m.notesMode = true
	m.groups = groupNotes([]*intent.Intent{note}, nil, nil, false)
	m.cursorGroup = 0
	m.cursorItem = 0
...
func TestRestore_TUIFlow_ArchivedNoteBecomesActive(t *testing.T) {
	...
	m.ready = true
	m.notesMode = true
	m.groups = groupNotes([]*intent.Intent{archived}, nil, map[string]bool{
...
func TestRestore_TUIFlow_NonArchivedNoOp(t *testing.T) {
	...
	m.ready = true
	m.notesMode = true
	m.groups = groupNotes([]*intent.Intent{note}, nil, map[string]bool{"notes": true}, false)
	if !selectNoteInGroups(&m, note.ID) {
		t.Fatal("note not found in groups")
	}
...
func TestUpdateNormal_MOnLifecycleIntentStillStatusPicker(t *testing.T) {
	...
	m := NewModel(ctx, nil, nil, "/tmp/intents", "", "", "", nil)
	m.ready = true
	m.groups = groupExplorerItemsByStatus([]*intent.Intent{item}, nil, false, nil, false)
