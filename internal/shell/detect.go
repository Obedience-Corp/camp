...
// FallbackShell is what camp suggests when it cannot identify the user's shell.
...
// DetectFromEnv names the shell camp should generate integration for, based on
...
// profile) to load camp's integration for the named shell. fish reads the
...
func InitCommand(shellType string) string {
	if shellType == "fish" {
		return "camp shell-init fish | source"
	}
	if !IsSupported(shellType) {
		shellType = FallbackShell
	}
	return `eval "$(camp shell-init ` + shellType + `)"`
}

// InitHint returns the init command for the shell camp detected from $SHELL.
