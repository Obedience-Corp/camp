...
func TestInitCommand(t *testing.T) {
	tests := map[string]string{
		...
		"nope": `eval "$(camp shell-init sh)"`, // unknown falls back to POSIX
	}
	for in, want := range tests {
		t.Run(in, func(t *testing.T) {
...
func TestInitHintFollowsShellEnv(t *testing.T) {
	...
	if got, want := InitHint(), `eval "$(camp shell-init sh)"`; got != want {
		t.Errorf("InitHint() under dash = %q, want %q", got, want)
	}

	t.Setenv("SHELL", "/usr/bin/fish")
	if got, want := InitHint(), "camp shell-init fish | source"; got != want {
