...
//
// Judgment is recorded for every row before reviewing, because the store
// refuses that transition while any row still lacks it. A helper that skipped
// it would only be able to test phases up to judging.
func advanceTo(t *testing.T, store *Store, phase Phase) *Run {
	...
	for _, step := range []Phase{PhaseSnapshotted, PhaseJudging, PhaseReviewing, PhaseApplying, PhaseVerified} {
		if step == PhaseReviewing {
			recordJudgmentForEveryRow(t, store, run)
		}
...
// recordJudgmentForEveryRow satisfies the judging -> reviewing gate.
func recordJudgmentForEveryRow(t *testing.T, store *Store, run *Run) {
	t.Helper()
	ctx := context.Background()
