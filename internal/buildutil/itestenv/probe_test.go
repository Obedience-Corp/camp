...
// The probe's whole purpose is refusing a run that cannot succeed, so the
// error paths are what matter: a daemon that never answers, one that dies
// partway, and one that answers too slowly to survive a suite.
func TestProbeResultDegraded(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			result:       ProbeResult{DockerHost: "unix:///d.sock", Err: camperrors.New("connection refused")},
			wantDegraded: true,
		...
		},
		{
			...
			result:     ProbeResult{DockerHost: "ssh://host", Err: camperrors.Wrap(errProbeUnsupported, `scheme "ssh"`)},
...
func TestProbeResultLine(t *testing.T) {
	...
	failed := ProbeResult{Err: camperrors.New("connection refused")}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if !camperrors.Is(err, errProbeUnsupported) {
...
// A stub daemon on a unix socket exercises the real request path without
// Docker: the probe has to measure a round trip and reject a non-OK answer.
func TestPingAgainstStubDaemon(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			// A unix socket path has a hard length limit (104 bytes on
			// Darwin), and the per-test temp directory is long enough to
			// exceed it, so the stub daemon listens under a short base.
...
func shortTempDir(t *testing.T) string {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "camp-itest-")
	if err != nil {
