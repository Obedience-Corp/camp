...
// Colima is the slice of the Colima CLI this package needs. It is an interface
// so the resolution decision table can be tested on machines where starting a
// VM is neither possible nor desirable.
type Colima interface {
	// Status reports one profile's state.
	Status(ctx context.Context, profile string) (ProfileStatus, error)
	// Start boots a profile, streaming Colima's own progress output to out.
	Start(ctx context.Context, spec StartSpec, out io.Writer) error
}

// commandRunner is the process-execution seam under the Colima adapter.
...
func (execRunner) Output(ctx context.Context, name string, args ...string) ([]byte, error) {
	...
	out, err := cmd.Output()
	if err != nil {
		return nil, camperrors.Wrapf(err, "%s %s: %s", name,
...
func (execRunner) Run(ctx context.Context, out io.Writer, name string, args ...string) error {
	...
	if err := cmd.Run(); err != nil {
		return camperrors.Wrapf(err, "%s %s", name, strings.Join(args, " "))
	}
	return nil
}
...
func (c colimaCLI) Status(ctx context.Context, profile string) (ProfileStatus, error) {
	if _, err := exec.LookPath(colimaBinary); err != nil {
		return ProfileStatus{Name: profile}, camperrors.Wrap(err, "find the colima CLI")
	}
	out, err := c.run.Output(ctx, colimaBinary, "list", jsonOutputFlag)
	if err != nil {
		return ProfileStatus{Name: profile}, camperrors.Wrap(err, "list Colima profiles")
	}
...
// parseColimaList reads `colima list --json`, which emits one JSON object per
// profile per line. A profile missing from the output has never been created,
// which is a state to report rather than an error: the caller creates it.
//
// Unparseable lines are skipped rather than failing the read. This output is
// diagnostic, and a future Colima release adding a line shape this does not
// know must not be able to stop the suite from running.
