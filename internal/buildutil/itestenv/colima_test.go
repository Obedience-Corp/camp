...
// fakeRunner records what would have been executed instead of executing it.
...
// The real `colima list --json` emits one object per profile per line.
...
{"name":"camp-itest","status":"Stopped","arch":"aarch64","cpus":6,"memory":8589934592,"disk":107374182400,"runtime":"docker"}`
	...
	tests := []struct {
		...
		},
		{
			...
			data: `{"name":"camp-itest","status":"RUNNING","cpus":6}`, profile: ProfileName,
...
// Starting the suite's profile must never move the user's Docker context: the
// residents on the default profile keep working while a run is in flight.
...
func TestColimaStartReportsFailure(t *testing.T) {
	...
	if !camperrors.Is(err, io.ErrClosedPipe) {
