...
const (
	...
	// (observability stacks, app databases, other tools' containers) stay on
	// the default profile: the point of a second VM is that nothing else lands
	// in it, so the pool's sizing model is true by construction.
	ProfileName = "camp-itest"

	// ProfileCPUs and ProfileMemoryGiB size the profile at creation. Six CPUs
	// carve out of a 16-core host while leaving the default profile's four
	// alone, and the pool clamps to six containers, so the VM can run a full
	// pool without oversubscribing itself.
	ProfileCPUs      = 6
	ProfileMemoryGiB = 8

	// ProfileDisabled is the CAMP_ITEST_PROFILE value that turns isolation off
	...
	// indistinguishable through os.Getenv, and "I meant to disable this" must
	// not be guessed.
	ProfileDisabled = "off"

	// EnvProfile overrides the profile name (or disables isolation entirely).
	EnvProfile = "CAMP_ITEST_PROFILE"
	...
	// profileStartTimeout bounds a cold VM boot. Creating this profile from
	// nothing measured 4m18s on 2026-08-16 (image download plus provisioning);
	// a warm start of an existing profile is well under a minute.
...
// daemon is exactly the failure mode this package exists to end, and it must
// never be discovered later from a collapse.
...
// fallback builds the shared-daemon result: whatever DOCKER_HOST already says,
...
// sameDockerHost compares two Docker host values by what they address rather
// than by spelling, so an inherited unix:///a/b and a freshly built one with a
// redundant separator are recognised as the same daemon.
