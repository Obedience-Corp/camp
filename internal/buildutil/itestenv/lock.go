...
const (
	...
	EnvLockWait = "CAMP_ITEST_LOCK_WAIT"

	// DefaultLockWait comfortably exceeds a healthy full-suite wall time, so
	// the common case (an agent session and a manual run colliding) resolves by
	// waiting rather than by failing someone's gate.
...
var (
	errLockBusy        = camperrors.New("lock is held by another process")
	errLockUnsupported = camperrors.New("file locking is unsupported on this platform")
)
...
// on each other. The daemon is the contended resource; the repository is not.
...
func profileLockPath(home, profile string) (string, error) {
	if profile == "" {
		return "", camperrors.New("profile name is required for the start lock")
	}
	return filepath.Join(home, obeyDirName, locksDirName,
...
// A doctor check must be able to answer "is a run in progress" without
// becoming the run in progress.
func LockStatus(path string) (held bool, description string) {
	...
	if err := lockFileNB(file); err != nil {
		if camperrors.Is(err, errLockBusy) {
...
type LockOptions struct {
	// Wait bounds how long to block for the lock (defaults to DefaultLockWait).
	Wait time.Duration
	// Out receives the waiting notices; without them a blocked run looks hung.
	Out io.Writer
	// Label names what is being protected, for the waiting notice.
	Label string
	// Poll is the retry interval (defaults to lockPollInterval).
	Poll time.Duration
...
//
// Two suite runs against one daemon do not merely run slower; they collapse
// each other, because each sizes its container pool as if it owned the VM.
// Serializing them is what makes a green run mean the suite passed rather than
// the suite happened to be alone.
func Acquire(ctx context.Context, path string, opts LockOptions) (*Lock, error) {
	...
	if err := os.MkdirAll(filepath.Dir(path), lockDirMode); err != nil {
		return nil, camperrors.Wrapf(err, "create lock directory for %s", path)
	}
	file, err := os.OpenFile(path, os.O_RDWR|os.O_CREATE, lockFileMode)
	if err != nil {
		return nil, camperrors.Wrapf(err, "open lock file %s", path)
	}
...
func waitForLock(ctx context.Context, file *os.File, path string, o LockOptions) (*Lock, error) {
	...
	for {
		...
		case camperrors.Is(err, errLockUnsupported):
		...
		case !camperrors.Is(err, errLockBusy):
			return nil, camperrors.Wrapf(err, "lock %s", path)
		}
		...
		if o.Now().After(deadline) {
			return nil, camperrors.Newf("%s is still locked after %s by %s; "+
...
func (l *Lock) Release() error {
	...
	l.file = nil
	if err != nil {
		return camperrors.Wrapf(err, "release lock %s", l.path)
	}
	return nil
}

// holder is the record a lock owner leaves for whoever waits behind it. A
// waiter that cannot name the holder leaves the reader guessing which of their
// terminals to go and look at.
type holder struct {
	PID     int       `json:"pid"`
