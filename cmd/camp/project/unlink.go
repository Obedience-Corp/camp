...
Examples:
  ...
  camp project unlink my-project --dry-run`,
	...
	RunE: runProjectUnlink,
}

func init() {
	Cmd.AddCommand(projectUnlinkCmd)

	flags := projectUnlinkCmd.Flags()
...
func runProjectUnlink(cmd *cobra.Command, args []string) error {
	...
	campaignResolver := newProjectCampaignResolver(cmd.ErrOrStderr(), "camp project unlink <name> --campaign <name>")
