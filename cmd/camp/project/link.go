...
func init() {
	Cmd.AddCommand(projectLinkCmd)

	flags := projectLinkCmd.Flags()
...
func runProjectLink(cmd *cobra.Command, args []string) error {
	...
	campaignResolver := newProjectCampaignResolver(cmd.ErrOrStderr(), "camp project link <path> --campaign <name>")
	_, root, err := campaignResolver.resolve(ctx, targetCampaign, cmd.Flags().Changed("campaign"))
	if err != nil {
		return err
	}

	result, err := linkProject(ctx, root, linkPath, name, path)
	if err != nil {
		return err
	}
