...
func TestSettingsLocalCommit_UnsetIsInheritAndClear(t *testing.T) {
	setupSettingsTest(t, true)

	// Unset local commit block reports inherit, not false.
	out, _, err := execSettingsCmd(t, newSettingsGetCmd, "local.commit.sync_project_refs")
	if err != nil {
		t.Fatalf("get unset local sync: %v", err)
	}
	if got := strings.TrimSpace(out); got != "inherit" {
		t.Fatalf("unset local.commit.sync_project_refs = %q, want inherit", got)
	}

	// Set a local override, then clear via inherit.
	if _, _, err := execSettingsCmd(t, newSettingsSetCmd, "global.commit.sync_project_refs", "true"); err != nil {
		t.Fatalf("set global sync: %v", err)
	}
	if _, _, err := execSettingsCmd(t, newSettingsSetCmd, "local.commit.sync_project_refs", "false"); err != nil {
		t.Fatalf("set local sync: %v", err)
	}
	out, _, err = execSettingsCmd(t, newSettingsGetCmd, "local.commit.sync_project_refs")
	if err != nil {
		t.Fatalf("get local sync: %v", err)
	}
	if got := strings.TrimSpace(out); got != "false" {
		t.Fatalf("local sync after set = %q, want false", got)
	}
	if _, _, err := execSettingsCmd(t, newSettingsSetCmd, "local.commit.sync_project_refs", "inherit"); err != nil {
		t.Fatalf("set local sync inherit: %v", err)
	}
	out, _, err = execSettingsCmd(t, newSettingsGetCmd, "local.commit.sync_project_refs")
	if err != nil {
		t.Fatalf("get after inherit: %v", err)
	}
	if got := strings.TrimSpace(out); got != "inherit" {
		t.Fatalf("local sync after inherit = %q, want inherit", got)
	}
	// Effective should fall back to global true.
	out, _, err = execSettingsCmd(t, newSettingsGetCmd, "effective.commit.sync_project_refs")
	if err != nil {
		t.Fatalf("get effective: %v", err)
	}
	if got := strings.TrimSpace(out); got != "true" {
		t.Fatalf("effective after inherit = %q, want true", got)
	}
}
