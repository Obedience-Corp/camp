...
func outputDungeonItems(items []intdungeon.DungeonItem, format string, outCtx dungeonListOutputContext) error {
	switch format {
	case "json":
...
func outputDungeonTable(items []intdungeon.DungeonItem, outCtx dungeonListOutputContext) error {
	if len(items) == 0 {
...
func outputDungeonSimple(items []intdungeon.DungeonItem) error {
	for _, item := range items {
